...
class PriceCalculator(View):
    ...
    def get(self, request):
        ...
        result = {
            "status": "failed",
            "message": "productCode must be required"
        }
        if request.GET["productCode"] != "":
            ...
            if request.GET["date"] != "":
                result = self.product_price(
                    ...
                    request.GET["date"],
                    request.GET.get("giftCardCode", None)
                )
    ...
    def product_price(self, product_code, date, gift_card_code):
        ...
        if len(product_price) == 1:
            if gift_card_code:
                gift_card_price = GiftCard.objects.filter(
                    Q(date_end=None) | Q(date_end__lte=date),
                    code=gift_card_code,
                    date_start__gte=date
                ).values("amount")
                result["message"] = "Invalid gift card code"
                if len(gift_card_price) == 1:
                    product_price = product_price[0]["price"]
                    gift_card_price = gift_card_price[0]["amount"]
                    price = product_price - gift_card_price
                    result.update({
                        "status": "success",
                ...
                price = json.loads(dumps(ProductPrice.objects.filter(
                    code=product_code,
                    offer_date=date
                ).values("price")))
                if len(price) == 1:
